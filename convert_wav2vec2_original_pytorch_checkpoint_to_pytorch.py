...
def set_recursively(key, value, full_name, weight_type, hf_pointer):
    # Handle new names of weight_g and weight_v
    def handle_old_weight_names(weight_type, hf_pointer):
        new_weight_type = PARAM_CONV_MAPPING[weight_type]
        for attribute in new_weight_type.split("."):
            hf_pointer = getattr(hf_pointer, attribute)
        logger.info(f"Processed {weight_type} by replacing it with {new_weight_type}")
        return hf_pointer
    ...
    if weight_type is not None and weight_type != "param":
        if weight_type in PARAM_CONV_MAPPING.keys() and not hasattr(hf_pointer, weight_type):
            hf_pointer = handle_old_weight_names(weight_type, hf_pointer)
    ...
    elif weight_type in PARAM_CONV_MAPPING.keys():
        if hasattr(hf_pointer, weight_type):
            hf_pointer = getattr(hf_pointer, weight_type)
...
def convert_wav2vec2_checkpoint(
    ...
    if is_finetuned or is_seq_class:
        model, cfg, _ = fairseq.checkpoint_utils.load_model_ensemble_and_task(
        ...
        model, cfg, _ = fairseq.checkpoint_utils.load_model_ensemble_and_task([checkpoint_path], task=task)
    model = model[0].eval()
    ...
    if is_seq_class:
        ...
        feature_extractor = Wav2Vec2FeatureExtractor(
            feature_size=1,
            sampling_rate=cfg['task']['sample_rate'], # Retrieve original value
            padding_value=0,
            do_normalize=bool(cfg['task']['normalize']), # Retrieve original value
